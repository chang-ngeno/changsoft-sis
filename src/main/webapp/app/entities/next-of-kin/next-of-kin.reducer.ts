...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_NEXTOFKIN: 'nextOfKin/PARTIAL_UPDATE_NEXTOFKIN',
...
export default (state: NextOfKinState = initialState, action): NextOfKinState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_NEXTOFKIN):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_NEXTOFKIN):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_NEXTOFKIN):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const getEntities: ICrudGetAllAction<INextOfKin> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<INextOfKin>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};
...
export const partialUpdate: ICrudPutAction<INextOfKin> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_NEXTOFKIN,
