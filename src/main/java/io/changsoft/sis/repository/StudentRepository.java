...
public interface StudentRepository extends JpaRepository<Student, Long> {
    @Query(
        value = "select distinct student from Student student left join fetch student.relatives",
    ...
    @Query("select distinct student from Student student left join fetch student.relatives")
    List<Student> findAllWithEagerRelationships();

    @Query("select student from Student student left join fetch student.relatives where student.id =:id")
